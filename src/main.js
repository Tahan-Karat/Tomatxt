...
function setupEventListeners() {
    ...
    if (editBtn) editBtn.addEventListener('click', toggleNoteEdit);

    document.addEventListener('submit', function (event) {
        if (event.target.id === 'subnote-form') addSubnote(event);
    });
    ...
    document.addEventListener('click', function (event) {
        ...
        if (noteCard && !event.target.closest('.note-menu-btn') && !event.target.closest('.note-menu')) {
            ...
            if (noteId) openNoteDetail(noteId).catch(console.error);
        }
    });
    ...
    document.addEventListener('keydown', (event) => {
        if (event.key === 'Escape') {
            ...
            if (noteDetail && noteDetail.getAttribute('aria-hidden') === 'false') closeNoteDetail();
        }
    });
}
